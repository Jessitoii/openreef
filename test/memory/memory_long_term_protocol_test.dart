...
void main() {
  ...
  setUp(() async {
    directory = await Directory.systemTemp.createTemp('openreef-memory-protocol-');
  ...
  test('long-term create rolls back when index rebuild fails', () async {
    ...
    expect(await storage.readRecord('lt-create', store: MemoryStoreKind.longTerm), isNull);
    await storage.close();
  });

  test('long-term edit without key change rolls back on index rebuild failure', () async {
    final storage = await openStorage();
    ...
    expect(result.status, MemoryMutationStatus.failedRolledBack);
    final reloaded = await storage.readRecord('lt-edit', store: MemoryStoreKind.longTerm);
    expect(reloaded?.content, 'Original content');
    expect(reloaded?.importance, 3);
    await storage.close();
  });

  test('long-term key change rolls back cleanly on index rebuild failure', () async {
    final storage = await openStorage();
    final original = MemoryRecord(
      store: MemoryStoreKind.longTerm,
      key: 'lt-old',
    ...
    expect(result.status, MemoryMutationStatus.failedRolledBack);
    expect(await storage.readRecord('lt-old', store: MemoryStoreKind.longTerm), isNotNull);
    expect(await storage.readRecord('lt-new', store: MemoryStoreKind.longTerm), isNull);
    await storage.close();
  });
  ...
  test('long-term delete rolls back when index rebuild fails', () async {
    ...
    expect(result.status, MemoryMutationStatus.failedRolledBack);
    expect(await storage.readRecord('lt-delete', store: MemoryStoreKind.longTerm), isNotNull);
    await storage.close();
  });
  ...
  test('store immutability is rejected below the UI layer', () async {
    ...
    final reloaded = await storage.readRecord('mutable-store', store: MemoryStoreKind.shortTerm);
    expect(reloaded?.store, MemoryStoreKind.shortTerm);
    await storage.close();
  });

  test('bulk delete deletes the full filtered set, not the visible cap', () async {
    final storage = await openStorage();
